...
MIN_VALID_STREAK = 3
MAX_SINGLE_OFFSET_M = 0.50
MAX_SINGLE_OFFSET_JUMP_M = 0.18
MIN_SINGLE_CURVATURE_M = 80.0
SINGLE_OFFSET_SMOOTHING = 0.35
MIN_SINGLE_VALID_STREAK = 3
    ...
    for pattern in ("/dev/v4l/by-id/*video-index0", "/dev/v4l/by-path/*usb*video-index0"):
        for link in sorted(glob.glob(pattern)):
            try:
                ...
                continue
            if target.startswith("/dev/video") and target not in usb_targets:
                usb_targets.append(target)
...
def existing_stable_video_links():
    ...
    for pattern in ("/dev/v4l/by-id/*", "/dev/v4l/by-path/*"):
        for link in sorted(glob.glob(pattern)):
            if "usb" not in link and "HD_Camera" not in link:
                continue
            try:
                target = os.path.realpath(link)
            except OSError:
...
def lane_metrics_from_fits(left_fit, right_fit, width, height, camera_status=1.0):
    ...
    lane_width_ratio = abs(right_bottom - left_bottom) / max(width, 1)
    offset_jump_m = (
        abs(offset_m - SINGLE_METRIC_STATE["offset_m"])
        if SINGLE_METRIC_STATE["valid_streak"] > 0
        else 0.0
    )
    candidate_valid = (
        right_bottom > left_bottom
        and 0.18 <= lane_width_ratio <= 0.85
        and abs(offset_m) <= MAX_SINGLE_OFFSET_M
        and curvature_m >= MIN_SINGLE_CURVATURE_M
        and offset_jump_m <= MAX_SINGLE_OFFSET_JUMP_M
    )

    if candidate_valid:
        alpha = SINGLE_OFFSET_SMOOTHING
        if SINGLE_METRIC_STATE["valid_streak"] > 0:
            offset_m = (1.0 - alpha) * SINGLE_METRIC_STATE["offset_m"] + alpha * offset_m
            curvature_m = (1.0 - alpha) * SINGLE_METRIC_STATE["curvature_m"] + alpha * curvature_m
        SINGLE_METRIC_STATE["offset_m"] = float(offset_m)
        SINGLE_METRIC_STATE["curvature_m"] = float(curvature_m)
        SINGLE_METRIC_STATE["valid_streak"] += 1
    else:
        SINGLE_METRIC_STATE["valid_streak"] = 0

    lane_detected = 1.0 if SINGLE_METRIC_STATE["valid_streak"] >= MIN_SINGLE_VALID_STREAK else 0.0
    return {
...
def annotate_lane_metrics(display_frame, metrics):
    ...
    cv2.putText(
        display_frame,
        f"Lane: {metrics['lane_detected']:.0f}  streak: {SINGLE_METRIC_STATE['valid_streak']}",
        (20, 130),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.7,
        (0, 255, 0) if metrics["lane_detected"] > 0.5 else (0, 0, 255),
        2,
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="OV9726 single USB camera LKAS lane tracking with UDP output."
    )
    parser.add_argument(
        "--camera",
        default="/dev/video0",
        help="OV9726 USB camera index or device path, for example /dev/video0",
    )
    parser.add_argument(
        "--left-camera",
        default=None,
    ...
    parser.add_argument(
        ...
        default="usb1",
        help="single-camera lane metrics source for Simulink",
    )
    parser.add_argument(
        "--usb1-boundary-start",
        type=float,
    ...
    if args.left_camera is not None or args.right_camera is not None:
        raise RuntimeError(
            "ov9726_single_lkas.py is for one camera only. "
            "Use lane_detection_preview.py for dual-camera experiments."
        )

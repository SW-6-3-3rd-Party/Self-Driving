...
/* PWM 주파수 = CLK_FREQ / PWM_PERIOD = 20MHz / 1000 = 20kHz
 * - 가청 영역(>20kHz 근처) 회피로 모터 소음 감소
 * - 전류 리플 감소(전기 시정수 L/R 대비 주기가 충분히 짧음)
 * - PERIOD=1000 유지 -> 듀티 분해능 1000스텝 그대로(Motor의 duty*10 호환)
 * ※ L298P(BJT)라 30kHz 이상은 스위칭 손실/발열 증가. 발열 시 18MHz로 낮춰 18kHz. */
#define CLK_FREQ            1000000.0f                         /* CMU clock frequency, in Hertz (20MHz)            */

...
/* ---- coast: 브레이크 해제 + PWM 0 (프리휠) ---- */
void Motor_coastChA(void) {
    GtmAtomPwmA_SetDutyCycle(0);
    MODULE_P02.OUT.B.P7 = 0;
}
void Motor_coastChB(void) {
    GtmAtomPwmB_SetDutyCycle(0);
    MODULE_P02.OUT.B.P6 = 0;
}

/* ---- short-brake: 브레이크 인가 + PWM=제동강도(0~100) ---- */
void Motor_brakeChA(int duty) {
    GtmAtomPwmA_SetDutyCycle(duty * 10);
    MODULE_P02.OUT.B.P7 = 1;   /* Brake on */
}
void Motor_brakeChB(int duty) {
    GtmAtomPwmB_SetDutyCycle(duty * 10);

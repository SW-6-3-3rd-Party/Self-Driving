function testFrontSteeringContract
%TESTFRONTSTEERINGCONTRACT Verify the Front TC375 steering wire contract.

packet = frontEncodeSteeringPacket(uint32(hex2dec('12345678')), ...
    uint64(123456789), single(0.25), single(1.0), uint8(1), uint8(5), ...
    uint16(hex2dec('ABCD')));

assert(isequal(size(packet), [40 1]));
assert(isequal(packet(1:4)', uint8('HPSC')));
...
expectedCrc = uint32(hex2dec('BDCB9681'));

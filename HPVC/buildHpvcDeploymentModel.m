function buildHpvcDeploymentModel(force)
%BUILDHPVCDEPLOYMENTMODEL Create the standalone HPVC UDP receiver model.
%
...
desktopModel = 'HPVCDesktopValidation';
deploymentModel = 'HPVCDeployment';
desktopFile = fullfile(rootDir, 'Models', [desktopModel '.slx']);
...
oldReceive = [deploymentModel '/UDP Receive from MIDDLE'];
front = [deploymentModel '/Decode and Monitor MIDDLE'];
deleteIncomingLines(front, 1:2);
delete_block(oldReceive);

receive = [deploymentModel '/UDP Receive from MIDDLE'];
...
add_block('simulink/Logic and Bit Operations/Compare To Constant', ...
    [deploymentModel '/Received 156 Bytes'], 'const', '156', 'relop', '==', ...
    'Position', [225 150 350 180]);

receivePorts = get_param(receive, 'PortHandles');
frontPorts = get_param(front, 'PortHandles');
sizeCheckPorts = get_param([deploymentModel '/Received 156 Bytes'], 'PortHandles');
...
% Do not inherit MATLAB's last-used board, which may be MIDDLE.
...
error('rccar:RaspberryPiBlocksetMissing', [ ...
    ...
    'Install it, restart MATLAB, and rerun buildHpvcDeploymentModel(true).']);
end
...
setMaskValue(block, 'local.*port', num2str(rccar.MiddleListenPort));
setMaskValue(block, 'data type', 'uint8');
setMaskValue(block, 'data size', '156');
...
sources = [steeringSource, frontPorts.Outport(3), ...
    ...
    get_param([model '/HPVC Local Time'], 'PortHandles').Outport(1), ...
...
encoder = [model '/Encode HPVC Diagnostic'];
add_block('simulink/User-Defined Functions/MATLAB Function', encoder, ...
...
setChartData(chart, 'controlValid', 'boolean', '1');
setChartData(chart, 'middleFlags', 'uint8', '1');
...
set_param(udpSend, 'remoteURL', ['''' rccar.HpvcDiagnosticHost ''''], ...
    'remotePort', num2str(rccar.HpvcDiagnosticPort), ...
...
script = sprintf([ ...
    'function packet = encodeDiagnostic(packetValid, linkValid, controlValid, middleFlags, sequence, packetAgeS, steeringCommand, localTimeS)\n' ...
    '%%#codegen\n' ...
    'packet=zeros(40,1,''uint8'');\n' ...
    'packet(1:4)=uint8([72;80;68;71]); packet(5)=uint8(1);\n' ...
    'status=uint8(packetValid)+bitshift(uint8(linkValid),1)+bitshift(uint8(controlValid),2);\n' ...
    'packet(6)=status; packet(7)=middleFlags; packet(8)=uint8(0);\n' ...
...
script = sprintf([ ...
    ...
    'packet=frontEncodeSteeringPacket(sequence,timestampUs,safeAngle,maxSteerRateRadS,controlMode,flags,aliveCount);\n' ...
